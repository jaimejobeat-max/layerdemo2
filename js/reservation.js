...
(function () {
  ...
  var MSG = {
    ko: { studio: '지점을 선택해 주세요.', part: '파트를 선택해 주세요.', date: '날짜를 확인해 주세요.', 'date-past': '지난 날짜는 신청할 수 없습니다.', time: '시작 시간이 종료 시간보다 앞서야 합니다.', 'min-hours': '최소 대관 시간보다 짧습니다.', company: '업체명(예약자명)을 입력해 주세요.', contact: '담당자를 입력해 주세요.', phone: '연락 가능한 전화번호를 입력해 주세요.', email: '이메일 형식을 확인해 주세요.', consent: '정보 수집에 동의해 주세요.', network: '전송에 실패했습니다. 잠시 후 다시 시도하거나 02-336-7750으로 연락해 주세요.', sending: '보내는 중…' },
  ...
  function fillParts(key) {
    ...
    sPart.innerHTML = '';
    if (!s) { sPart.innerHTML = '<option value="">' + (window.LANG === 'en' ? 'Choose a studio first' : '지점을 먼저 선택') + '</option>'; return; }
    var parts = s.parts.length ? s.parts : ['-'];
    if (parts.length > 1) sPart.insertAdjacentHTML('beforeend', '<option value="">' + (window.LANG === 'en' ? 'Select' : '선택') + '</option>');
    parts.forEach(function (p) { sPart.insertAdjacentHTML('beforeend', '<option value="' + p + '">' + p + '</option>'); });
    sPart.disabled = false;
  }
  sStudio.addEventListener('change', function () { fillParts(sStudio.value); });
  var pre = new URLSearchParams(location.search).get('studio');
  if (pre && studios.some(function (s) { return s.key === pre; })) { sStudio.value = pre; }
  fillParts(sStudio.value);
  var today = new Date(Date.now() + 9 * 3600e3).toISOString().slice(0, 10); dDate.min = today;
  sStart.value = '9'; sEnd.value = '18';
  document.addEventListener('langchange', function () { var v = sPart.value; fillParts(sStudio.value); sPart.value = v; document.getElementById('rLang').value = window.LANG; });
